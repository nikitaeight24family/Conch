...
Conch is a native Android client that connects to **your own** SSH servers and runs your chosen AI agent (Claude Code, Codex CLI, Gemini CLI, Grok Build, GitHub Copilot CLI, Qwen Code, Cursor CLI, opencode, Crush, or Continue CLI) on them. **No data the app handles leaves your device or your servers.** There is no analytics, no crash reporting and no telemetry of any kind, and Conch has no backend to send anything to. The only network connections the app opens are the SSH connections to the servers you add yourself — plus, for the on-device local models feature only: downloading a model file you chose from Hugging Face, and, when you open the model store screen, read-only fetches of the model catalog (from Conch's public GitHub repository) and of public download/like counts (from Hugging Face's public API). These requests send no account, device, or usage data — they are plain anonymous GETs, and everything the store knows about your phone (RAM, chip, measured speeds, your ratings) stays on the phone.
