...
internal object RemoteTurnKiller {
    ...
    fun killScript(resumeId: String): String {
        ...
        return "rid='" + resumeId + "'; " +
            ...
            "awk '\$2 !~ /(^|\\/|-)((ba|da|a|z)?sh)\$/ && /(claude|codex|gemini|grok|copilot|qwen|cursor-agent|opencode|crush)/ {print \$1}'); " +
            "if [ -z \"\$pids\" ]; then " +
            "pids=\$(pgrep -f \"(claude|codex|gemini|grok|copilot|qwen|cursor-agent|opencode|crush).*\$rid\" 2>/dev/null | grep -v \"^\$\$\\\$\"); fi; " +
