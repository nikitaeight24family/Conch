...
warning row and a re-send. It now just continues — and for Codex, both sides can
hold the same session at once.

### Added
- **A send claims the session.** When a terminal on the server holds the session
  you are writing into, the app ends that copy, resumes the same session and
  delivers the message in the same turn, then says so in one line. The history is
  never at risk: it lives in the session file, not in the process that held it.
- **One `codex app-server` for both sides.** Conch starts a single app-server on
  the server's loopback and connects to it over the SSH transport it already
  holds — nothing listens on the phone, and no sshd change is required (a
  forwarded port is refused by ordinary `PermitOpen` hardening, so the bytes ride
  an exec channel instead). Two clients of one app-server share a thread, so the
  phone and the terminal stop fighting over the writer. The terminal is wired to
  join it automatically.
- **Every agent claims its session, not just two.** The one-shot path (eight of
  the ten CLIs) and the Gemini channel now run the same check before resuming:
  our own orphaned processes are cleaned up silently, a person's terminal is only
  ended because a send claimed the session, and anything unprovable is left
  alone.

### Fixed
- **Gemini sessions resume again.** Gemini files sessions under a hash of the
  working directory and refuses outright from anywhere else — "no previous
  sessions found for this project", exit 42. The directory is now recovered from
  the CLI's own project map plus the session file's full id, and all three resume
  paths share that recovery instead of two of them having it.
- **A held Codex thread no longer loops.** Treating it as a dropped connection
  sent it through the reconnect ladder: relaunch, refuse, relaunch, a wall of
  "starting codex" every three seconds. It now parks quietly and carries on.
- A shared app-server that accepts the connection but never answers no longer
  hangs the turn — the channel gives up after eight seconds and a private
  app-server takes over.
- Codex's own internal remote-control events no longer print as chat rows.
